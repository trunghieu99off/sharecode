...
$id_get = (int)$_GET['id'];
$info = mysqli_fetch_assoc(mysqli_query($connect, "SELECT * FROM danhmuc WHERE id = '$id_get'"));
		...
		<div class="row">
			...
			if(isset($_POST['them'])){
				...
				if( !empty($_POST['ten']) && !empty($_POST['mota']) && !empty($_POST['thumbnail']) ){
					...
					mysqli_query($connect, "UPDATE danhmuc SET ten = '$ten', mota = '$mota', thumbnail = '$thumbnail' WHERE id = '$id_get'");
